# Lock Convoys: How They Form and How to Avoid Them (Claude Opus 4.6)

*A conversation with Claude Opus 4.6: where lock convoys show up in practice, how a single preemption turns into a self-sustaining lockstep queue, why it doesn't recover on its own, and how jitter, shorter critical sections, try-lock with backoff and partitioning break it.*
...
<strong style="font-size: 1.4em;">Q:</strong>&nbsp; Can you tell me about most frequent manifestations of the "lock convoy" problem in multithreaded/parallel programming?
</td></tr>
</table>

The **lock convoy** problem occurs when multiple threads compete for the same lock and end up serializing their execution in a way that destroys parallelism and throughput. Here's how it typically manifests:
...
<table style="margin: 32px 0 20px auto; max-width: 75%; border-collapse: separate; border: none;">
<tr><td style="background: #1e3f73; color: #ffffff; border: none; border-radius: 20px; padding: 12px 18px; line-height: 1.5;">
<strong style="font-size: 1.4em;">Q:</strong>&nbsp; So, essentially, as you describe it, lock convoy happens mostly b/c of high contention for a shared resource.. which creates a "serialization" point for all the threads, essentially killing concurrent progress. Right?
</td></tr>
</table>

Yes, that's the core of it - but there's a subtle distinction worth drawing between general lock contention and a true convoy.
...
<table style="margin: 32px 0 20px auto; max-width: 75%; border-collapse: separate; border: none;">
<tr><td style="background: #1e3f73; color: #ffffff; border: none; border-radius: 20px; padding: 12px 18px; line-height: 1.5;">
<strong style="font-size: 1.4em;">Q:</strong>&nbsp; I don't feel like you have described the genesis of the problem in enough detail/clearly enough. Can you focus on that? And especially the "self-sustaining/reinforcing" quality of it?
</td></tr>
</table>

You're right, let me walk through it more carefully.
...
<table style="margin: 32px 0 20px auto; max-width: 75%; border-collapse: separate; border: none;">
<tr><td style="background: #1e3f73; color: #ffffff; border: none; border-radius: 20px; padding: 12px 18px; line-height: 1.5;">
<strong style="font-size: 1.4em;">Q:</strong>&nbsp; 1. Would an occasional random (short) wait in a thread before attempting/releasing the lock help?
<br>
